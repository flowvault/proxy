 ...
 * apibuilder:0.12.38 https://app.apibuilder.io/flow/organization/0.3.47/play_2_5_client
 */
package io.flow.organization.v0.models {
   ...
   * @param role
   * @param id
   * @param expiration The date / time on which this token expires. Current default
   * is for tokens to live for 1 week
   */
   ...
   * @param environment
   * @param parentId When creating an organization in the test environment, you must
   * specify your production organization id.
   */
   ...
   * @param name Defaults to the id
   * @param environment If updating, defaults to the current environment. If
   * creating, defaults to production
   * @param parentId When creating an organization in the test environment, you must
   * specify your production organization id.
   */
...
package io.flow.organization.v0 {
  ...
  object Constants {
    ...
    val UserAgent = "apibuilder:0.12.38 https://app.apibuilder.io/flow/organization/0.3.47/play_2_5_client"
    val Version = "0.3.47"
    val VersionMajor = 0
    ...
    object Invitations extends Invitations {
      ...
      override def putTokensByToken(
        ...
        _executeRequest("PUT", s"/invitations/tokens/${play.utils.UriEncoding.encodePathSegment(token, "UTF-8")}", requestHeaders = requestHeaders).map {
          ...
          case r if r.status == 401 => throw new io.flow.organization.v0.errors.UnitResponse(r.status)
          case r if r.status == 404 => throw new io.flow.organization.v0.errors.UnitResponse(r.status)
          case r if r.status == 422 => throw new io.flow.organization.v0.errors.InvitationErrorResponse(r)
  ...
  trait Organizations {
     ...
     * @param parent Find organizations that belong to this specified parent
     * organization id. Case insensitive. Exact match.
     * @param limit
     * @param offset
     * @param sort
     */
